...
public partial class UserDefinedFunctions
{
    ...
    public static int MatchCountString(String strInput, String strPatternValue, Boolean ignoreCase)
    {
        int defaultMatch = 0;
        ...
        if ((strInput != null) || (strPatternValue != null))
        {
            ...
            String strInputclean = null;
            String strPatternValueclean = null;

            strInputclean = Regex.Replace(strF, @"[\d-]", string.Empty);
            strPatternValueclean = Regex.Replace(strS, @"[\d-]", string.Empty);


            List<string> stringsList = new List<string>(strInputclean.Split(new string[] { " " }, StringSplitOptions.RemoveEmptyEntries));

            int count = 0;
            foreach (string item in stringsList)
            {
                //Ignora palavras isoladas 
                if (Regex.IsMatch(strPatternValueclean, string.Format(@"\b{0}\b", Regex.Escape(item))))
                    count++;
            }
